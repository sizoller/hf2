...
\begin{circuitikz}[american currents,american voltages,line width=0.25mm]
	...
	\draw (QA.B) to (\alX  + \comD ,\alY - \comD) node[circ]{} to (\alX + 0.5 + \comD ,\alY - \comD) node[ground, rotate=-180](alC){};		%draw substrate ground from FET A
	\draw (QB.B) -| (alC);											%connect bulk from FET B to substrate	
